...
double dBiCop_cpp(const double& uu,
  ...
  // Clayton
    ...
    stop("error: this family is not available.");
...
double dBiCopMar_cpp(const double& xx,
  ...
  if (marg_family == 3) {
    ...
    uu = pCEpois(xx, mar_param(0));
    vv = pCEpois(yy, mar_param(1));
    
  }
  else if (marg_family == 4) {
    
    uu = pCEnbinom(xx, mar_param(0), mar_param(1));
    vv = pCEnbinom(yy, mar_param(2), mar_param(3));
...
arma::colvec dBiCopMar_cpp2(const arma::colvec& xx,
  ...
  if (marg_family == 3) {
    ...
    for (int i = 0; i < nn; ++i) {
      ...
      uu(i) = pCEpois(xx(i), mar_param(i,0));
      vv(i) = pCEpois(yy(i), mar_param(i,1));
    ...
    for (int i = 0; i < nn; ++i) {
      ...
      uu(i) = pCEnbinom(xx(i), mar_param(i,0), mar_param(i,1));
      vv(i) = pCEnbinom(yy(i), mar_param(i,2), mar_param(i,3));
...
// [[Rcpp::export]]
List updateFullCopLabel(const arma::colvec& yy,
  ...
  for (int i = 2; i < nne; ++i) {
    ...
    iyy.set_size(i);
    iyy.fill(yy(i));
    iyy_ne = arma::trans(yy_ne(i, arma::span(0, i - 1)));
    irho = arma::trans(rho(i, arma::span(0, i - 1)));
    
    // if (strcmp(marg_family, "poisson") == 0) {
    if (marg_family == 3) {
      iyy_param1.set_size(i);
      iyy_param1.fill(yy_param1(i));
      mar_param.set_size(i, 2);
      mar_param.col(0) = iyy_param1;
      mar_param.col(1) = arma::trans(yy_ne_param1(i, arma::span(0, i - 1)));
    } else {
      iyy_param1.set_size(i);
      iyy_param2.set_size(i);
      iyy_param1.fill(yy_param1(i));
      iyy_param2.fill(yy_param2(i));
      mar_param.set_size(i, 4);
      mar_param.col(0) = iyy_param1;
      mar_param.col(1) = iyy_param2;
      mar_param.col(2) = arma::trans(yy_ne_param1(i, arma::span(0, i - 1)));
      mar_param.col(3) = arma::trans(yy_ne_param2(i, arma::span(0, i - 1)));
    }

    iloglik = dBiCopMar_cpp2(iyy, iyy_ne, cop_family, irho, marg_family, mar_param, true, true);
    ...
    ilabel_val = arma::linspace<arma::colvec>(1, i, i);
    ilabel = arma::as_scalar(armaIntSample(ilabel_val, 1, 1, idistr));
    labels(i) = ilabel;
    
    // Update latent variables from the truncated Gaussian
  ...
  for (int i = nne; i < data_size; ++i) {
    ...
    // if (strcmp(marg_family, "poisson") == 0) {
    if (marg_family == 3) {
      mar_param.set_size(nne, 2);
      iyy_param1.fill(yy_param1(i));
      mar_param.col(0) = iyy_param1;
      mar_param.col(1) = yy_ne_param1.row(i).t();
    } else {
